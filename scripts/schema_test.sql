-- Test database schema. Mirrors scripts/schema.sql exactly, but targets
-- invoiceiq_test instead of invoiceiq, so automated tests never touch
-- real production data. Run once (or after a full wipe) before running
-- the pytest suite.

IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'invoiceiq_test')
BEGIN
    CREATE DATABASE invoiceiq_test;
END
GO
